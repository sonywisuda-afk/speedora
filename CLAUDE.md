...
- **Editorial Director Phase A** ("Editorial Intelligence 2.0") — the first phase of a user-supplied
  ...
  source-signal mappings, still HEURISTIC (ADR D4) pending real engagement data. Phases B (Edit
  Orchestration/Budget/Conflict Resolver), C (Render QA/auto-reject/fallback), and D (real-video
  benchmark/human evaluation) are the roadmap's next phases, explicitly not started. See
...
- **Edit Plan Director Phase B** ("Edit Orchestration") — the second phase, building on Phase A and
  ...
  real `nest build`). Phase C (Render QA/auto-reject/fallback) and D (real-video benchmark/human
  evaluation) remain next, explicitly not started. See `ai/edit-plan-director.md`.
