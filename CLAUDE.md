...
| [`docs/ai/render-fidelity-matrix.md`](docs/ai/render-fidelity-matrix.md) | Render Fidelity & Composition Execution Engine — Phase 0 audit for a proposed unification of every render-affecting setting/AI-decision behind one deterministic `EffectiveRenderConfig → RenderPlan → FFmpeg Compiler → Render Manifest → ffprobe verification` pipeline. Four parallel read-only audits (UI/API/persistence/queue-payload, worker core pipeline, captions/visual-emphasis/B-roll/branding/transitions, clip-count/duration funnel) produced a full feature-to-render matrix and found **no unified config object exists today** (each setting resolved by its own independent `resolveX()` helper in `render-clip.worker.ts`, threaded as a loose param) plus **6 concrete, independently-fixable bugs**: `concatBrandSegment()` (intro/outro, the last encode pass) has no `quality` param so it silently drops the user's CRF/preset; it and `trimAndFadeInBRoll()` also hardcode 30fps even for 60fps sources; `applyReactionHolds()`/`concatBrandSegment()` hardcode 44.1kHz/stereo instead of reusing `resolveAudioEncodeArgs()`, silently upmixing mono sources; `selectShortlist()` is called without `targetSize` so it always caps at the hardcoded default of 15 regardless of a larger requested `clipCount`; no production code ever ffprobes a rendered clip's actual final duration and reconciles it against stored `Clip.durationSeconds`; and no dedup/diversity check exists between candidates in the same generation batch on the initial upload flow. Also found several things already correctly built that don't need rebuilding (aspect ratio is not hardcoded, captions already use final output dimensions not source, Visual Emphasis's crop/zoom techniques already funnel into one composition plan, candidate-expansion's `maxCandidates` prompt wiring is already fixed contra the stale claim in `clip-ranking-engine.md`). Recommended splitting the 6 bugs (small, shippable now) from the larger `RenderPlan`/compiler architecture (multi-week, needs its own phase-gated initiative) rather than building both under one unscoped mandate — a scope decision put to the user, not resolved unilaterally in the audit. **All 6 bugs shipped** in one pre-Phase-1 fix commit. **Phases 1-9 of the larger architecture are now ALL SHIPPED**: Phase 1 `EffectiveRenderConfig`, Phase 2 `OutputProfile`, Phase 3 `RenderPlan` (`packages/contracts`/`packages/render-config`, proof-of-integration-only — computed and logged on every render, not yet driving execution), Phase 4 the FFmpeg Execution Compiler (`apps/worker/src/render-plan-compiler.ts`, pure/deterministic, `import type`-only ffmpeg.ts imports so it structurally cannot execute anything), Phase 5 the actual cutover executor (`apps/worker/src/execute-render-plan.ts`) wired in behind `RENDER_EXECUTION_COMPILER_ENABLED` (default `false`, confirmed nowhere enabled in this repo — the inline pipeline remains the only real production execution path), Phase 6 a rollout runbook (`docs/ai/render-fidelity-rollout.md`, documentation only, no rollout has actually happened), Phase 7 `RenderManifest` and Phase 8 ffprobe verification (`RenderVerificationResult` — both logged on every render, deliberately excluding `pixelFormat`/duration from comparison scope, neither yet exposed via any API/UI), and Phase 8 also shipped a real, unconditional bug fix (`resolveAudioEncodeArgs()` now always sets `-ar 44100`, closing the exact "renderClip()-only renders silently keep the source's native sample rate" gap the audit's own bug list implied but didn't separately call out). Phase 9 became the **Clip Count & Duration Precision Engine**'s duration half (`docs/ai/clip-duration-precision-engine.md` — clip *count* precision, i.e. bug #4's `selectShortlist()` `targetSize` cap, was separately already addressed by `clip-ranking-engine.md`'s own Phase 13.1, not by this initiative): a new `Clip.renderedDurationSeconds` column, additive/nullable, persisted atomically with `outputUrl` by the pre-existing optimistic-concurrency completion transaction, sourced from ONE consolidated `probeVideoMetadata()` call shared with Phase 8's own verification step (collapsing what had briefly been two independent ffprobe calls against the same file). Deliberately distinct from the pre-existing `Clip.durationSeconds` (the requested/candidate window) — see that doc's own lifecycle semantics for why a manual trim never touches the new field. No historical backfill for already-rendered clips (matches the `outputWidth`/`outputHeight` precedent), no UI exposure yet. See [`docs/ai/render-fidelity-rollout.md`](docs/ai/render-fidelity-rollout.md) for the Phase 6 rollout runbook and [`docs/ai/clip-duration-precision-engine.md`](docs/ai/clip-duration-precision-engine.md) for the Phase 9 audit/design. |
| [`docs/ai/render-fidelity-rollout.md`](docs/ai/render-fidelity-rollout.md) | Render Fidelity & Composition Execution Engine, Phase 6 — a documentation-only production rollout runbook for `RENDER_EXECUTION_COMPILER_ENABLED` (Phase 5), written before any real rollout was attempted (and before it was discovered Speedora has no real production deployment at all yet). Documents two strategies: instance-based canary (zero new code, reuses Fase 3's existing per-instance env layering, but requires verifying ≥2 live `render-clip` worker instances first — not assumed) and a future percentage-based rollout (`RENDER_EXECUTION_COMPILER_ROLLOUT_PERCENT`, deterministic `clipId`-hash based, explicitly NOT implemented — described only, its own separate future phase, deliberately still deferred as of the rollout-planning pass below). **Rollout-planning pass (2026-08-15)**: added a new §0 "Rollout sequence" — `LOCAL EQUIVALENCE GATE → Production deployment → Production smoke test → Canary/full rollout` — the real gate order given no deployment exists yet, fulfilling an explicit instruction from the equivalence gate's own approval that had been missed until this pass (confirmed via `git log` on this file — it had never been touched since its original Phase 6 commit). References `docs/ai/render-fidelity-local-equivalence-gate.md`'s own PASSED result as the gate this runbook's canary procedures now sit behind, and adds `GET /clips/:id/render-fidelity` (Path B) to §5's observability list as a richer, persisted, per-clip alternative to grepping structured logs. No rollout has actually happened; the flag remains `false` everywhere. |
| [`docs/ai/clip-duration-precision-engine.md`](docs/ai/clip-duration-precision-engine.md) | Render Fidelity & Composition Execution Engine, Phase 9 — a full audit of `Clip.durationSeconds`'s entire lifecycle (every writer/consumer, 10 specific questions answered against real code, not assumed) followed by a design revision resolving the two ambiguities the audit surfaced, both completed and reviewed before any code was written. Shipped: `Clip.renderedDurationSeconds`, the real post-render duration, additive/nullable, atomic with `outputUrl`, sourced from one consolidated ffprobe probe shared with Phase 8's own verification (no second, independent probe). Explicitly out of scope: clip *count* precision (a separate, already-addressed concern — see `clip-ranking-engine.md`), any change to `durationSeconds`' own meaning, `RenderManifest`/`RenderVerificationResult` schema changes, UI exposure, historical backfill. |
| [`docs/ai/render-fidelity-local-equivalence-gate.md`](docs/ai/render-fidelity-local-equivalence-gate.md) | Render Fidelity & Composition Execution Engine — a local Real-Output Equivalence Gate for the compiled executor (Phase 5), built as Path A's practical substitute once it was discovered Speedora has no live production deployment yet (so the rollout runbook's own instance-based canary doesn't apply). `apps/worker/src/workers/render-clip.worker.local-comparison.ts` invokes the literal production processor closure twice per scenario (compiler flag off, then on) against identical `job.data`, with real ffmpeg/ffprobe and real `@speedora/render-config`/`render-plan-compiler` — only AI detectors/storage/prisma mocked; manual-only (doesn't end in `.spec.ts`, excluded from `pnpm test`/`pnpm build`), 5 scenarios × real dual-run. **Result: 5/5 scenarios PASS, 10/10 real renders**, every field identical legacy vs compiled, full 5-pass sequence confirmed for the Full pipeline scenario — reusable as the real smoke test/canary once a deployment exists. Also documents one real, isolated-and-measured finding from that work: `concatBrandSegment()`'s two intro/outro re-encodes each add a small (~15-40ms), AAC-encoder-delay-driven duration overshoot that eventually shows up as a genuine (not metadata-only) ~1-frame-period stretch in Phase 8's own fps verification — confirmed via real per-frame PTS measurement to be bounded/non-growing and not a dropped/duplicated-frame bug, accepted and documented rather than fixed (same posture as `silence-compression.md`'s Reaction Hold × crossfade interaction), pending real-footage calibration ever surfacing it as perceptible. |
| [`docs/ai/editorial-director.md`](docs/ai/editorial-director.md) | Editorial Director Phase A ("Editorial Intelligence 2.0") — the first phase of a user-supplied 30-section "Speedora Editorial Operating System" mission aimed at moving clip quality toward Opus Clip parity by making the pipeline behave like one editorial decision-maker instead of independently-triggering signals (diagnosis: "feature-rich, decision-poor"). A 4-way parallel code audit confirmed real double-counting in `packages/clip-ranking`'s inert `compositeRank` (`virality`/`hook`/`narrative`/`retention` share common ancestors; 6 `ClipScores` fields are also folded into `fusion` a second time), zero semantic dedup/diversity/narrative-boundary logic anywhere, and that every AI Intelligence v4 signal already computes on every render regardless of its own flag (ADR D8) — Phase A is built almost entirely as an orchestration layer over those already-shipped signals, no new detector. **Shipped**: new `packages/editorial-director` (no LLM dependency) producing one `EditorialDecision` in two modes — `mode: 'shortlist'` (pre-render, inside `@speedora/candidate-shortlist`, ungrounded transcript-only signals; real behavior changes: negative-signal penalties, a bounded ±3s-heuristic boundary nudge requiring BOTH a lexical and a structural signal to agree, and diversity-aware re-selection via keyword/n-gram Jaccard + semantic-event-type overlap — a user-confirmed scope decision over real embeddings) and `mode: 'render'` (post-render, inside `render-clip.worker.ts` right after the render graph resolves, using the full grounded v4 signals plus `editingSuggestions` density/clustering; computed always per ADR D8, written to new additive `Clip.editorialDecision`, scoring/observability only this phase — does not gate rendering). Fixes clip-ranking's double-counting structurally by composing 8 named categories (Content Value/Hook Strength/Narrative Completeness/Context Completeness/Emotional Payoff/Visual Engagement/Speaker Clarity/Platform Fit) with documented non-overlapping source-signal mappings instead of averaging correlated raw sub-scores, each still HEURISTIC/uncalibrated (ADR D4) pending real engagement data. Also fixed, while wiring this phase: a real pre-existing bug in `@speedora/virality-engine`'s `deriveNarrativeGraphScore`/`deriveSemanticEventsScore` (strict `=== null` checks crashing on a render-graph mock's `undefined`, the same bug class Phase 9 already fixed once elsewhere in the same package). Editorial Director only activates on the shortlist path that already pays for its two LLM calls today (never forces new cost onto the common small-pool case) — a user-confirmed scope decision. Explicitly out of scope for this phase: Edit Plan Director/Budget/Conflict Resolver (Phase B), pre/post-render Quality Gate/auto-reject/fallback (Phase C), real-video benchmark/human evaluation (Phase D), grounded post-render boundary re-cut, real embeddings, speaker-role inference, and fixing/removing `clip-ranking`'s own still-inert `compositeRank` (documented, deliberately left alone). |
| [`docs/ai/edit-plan-director.md`](docs/ai/edit-plan-director.md) | Edit Plan Director Phase B ("Edit Orchestration") — the second phase of the same "Speedora Editorial Operating System" mission, building on Phase A. Builds on a prior, separate initiative's own quantified evidence (`docs/ai/visual-emphasis-integration-audit.md`'s Gate B: 531px/s peak crop velocity and a 160px/0.4s displacement for Focus Shift×Digital Push overlapping at their worst case; OCR Highlight's static-snapshot drift measured 3.7%→33%→40%+/"leaves frame" across movement magnitudes, coupled to that same finding; Reaction Hold×Pause Hold confirmed an intentional, correct-by-design product dependency, not a bug) — that audit explicitly declined to build an arbitration rule pending real footage; user-confirmed this phase is a different question (building the governing mechanism now, informed by real numbers, not the calibration question Gate B declined to answer). **Shipped**: new `packages/edit-plan-director` (no LLM dependency) — an adaptive per-technique Edit Budget (duration-scaled, halved when Phase A's own `EditorialDecision.negativeSignals` already flagged visualInstability/overEditingRisk, zeroed for speaker_focus_shift on monologue clips) plus an Effect Conflict Resolver implementing 3 pairwise rules grounded in Gate B's evidence (Focus Shift×Digital Push time-overlap → suppress the digital_push trigger, "REDUCE INTENSITY" implemented as suppression rather than continuous magnitude damping since `buildCropPath()` has no per-trigger intensity channel; OCR Highlight × a surviving shift/push overlap → suppress the highlight; Reaction Hold×Pause Hold → kept, observability-only, no code change) plus budget enforcement (drops lowest-`score` suggestions first, finally giving `EditingSuggestion.score` a real consumer). Time overlap is used as the conflict trigger throughout, not a computed velocity estimate — a named, honest limitation, since this layer only ever sees the abstract suggestion timeline, never the actual crop-path pixel positions Gate B's own measurements came from. Wired as ONE arbitration point (`planEdits()`, right after the render graph resolves, reusing Phase A's already-computed duration/speaker-count/negative-signal inputs) feeding ALL FOUR real downstream consumers (`buildReframePlan()`, `computeClipCuts()`, and both Reaction Hold call sites - confirmed by audit that Pause/Reaction Hold read `graphResult.editingSuggestions` directly, bypassing `buildReframePlan()` entirely), not just the one call site a naive interception would cover. New additive `Clip.editPlan` column (budget + decision trail, `suggestions` itself not duplicated - already covered by `Clip.editingSuggestions`). Hit the documented TS2742 Prisma pitfall a second time while wiring this phase (same fix pattern as Phase A, verified with a real `nest build` both times). Explicitly out of scope: true continuous zoom-magnitude damping (would require extending the already-shipped, real-ffmpeg-verified `buildCropPath()` itself), caption/B-roll conditional-inclusion logic, a full cross-domain EditPlan spanning every editorial domain, and Gate C real-footage calibration of the thresholds chosen here. |
| [`docs/ai/render-quality-judge.md`](docs/ai/render-quality-judge.md) | Render Quality Judge Phase C1 ("Render QA," data only) — the third phase of the same "Speedora Editorial Operating System" mission, building on Phases A/B. The mission's original Section 17 ask was a full auto-reject + safe-fallback re-render loop; a 3-way parallel audit (pre-render validation hooks, post-render quality measurement infrastructure, retry/re-render mechanics) found that ask isn't safely buildable in one pass — the 45-minute job timeout is sized for exactly one render pass with no headroom for a second, `renderClip()` is a monolithic non-reusable ffmpeg call, no `Clip` status/verdict column exists anywhere, and most of the named Quality Judge inputs (VisualQuality/AudioQuality/CaptionQuality) have only weak indirect proxies or nothing at all (confirmed: no transcript-to-audio alignment mechanism exists anywhere in this codebase). User-confirmed scope (via `AskUserQuestion`): Phase C1 ships an additive, OBSERVATIONAL `FinalClipQualityAssessment` only — no gating/reject/re-render. **Shipped**: new `packages/render-quality-judge` (no LLM dependency) — 6 dimensions (editorialQuality/narrativeQuality/technicalQuality composed from already-computed EditorialDecision + probeVideoMetadata()/RenderVerificationResult/duration-drift, all real `'measured'` signals; visualQuality/audioQuality as explicitly-labeled `'proxy'` scores reusing visualEngagement/speakerClarity, which measure something adjacent, not true picture/audio quality; captionQuality always `'unavailable'`, a confirmed honest gap, never fabricated) composed into one `compositeScore` (measured weight 1, proxy weight 0.5) plus a coverage `confidence` that discounts proxy-heavy coverage. New additive `Clip.qualityAssessment` column, always computed and persisted regardless of `RENDER_QUALITY_JUDGE_ENABLED` (ADR D8 - flag gates future API exposure only), wired right after `renderVerification` resolves in `render-clip.worker.ts` reusing every input from Phase A/B/Render Fidelity's already-hoisted locals - no new I/O. Hit the documented TS2742 Prisma pitfall a third time while wiring this phase (same fix pattern as Phase A/B, verified with a real `nest build`). Explicitly out of scope, named as Phase C2 candidates: auto-reject/re-render (needs a status column + a second render budget the timeout doesn't have), real Visual/Audio/Caption quality detectors (genuinely new detection work against zero real calibration data), and Gate C real-footage calibration of this phase's own heuristic weights. |
| [`docs/ai/phase-d-benchmark.md`](docs/ai/phase-d-benchmark.md) | Phase D ("Real-video benchmark, blind human evaluation") — the fourth and final phase of the same "Speedora Editorial Operating System" mission, closing out Phases A/B/C1. Split into an automated benchmark (buildable) and a blind human evaluation session (not something an agent can perform) via `AskUserQuestion` - scope resolved to: build the benchmark harness + a structured human-review packet, leave the actual evaluation to a human. Audit found real video/DB data already live in this dev environment (3 real videos, 4 clips), a previously-undocumented blocker (Phase A/B/C1's own migrations had never been applied to this local Postgres - fixed as a prerequisite step), that only `EDIT_BUDGET_ENABLED`/`EFFECT_CONFLICT_RESOLUTION_ENABLED` change the physical rendered file, and that `getProcessor()`'s real-processor-closure-capture technique only works inside Jest (not a plain tsx script). **Shipped**: `render-clip.worker.phase-d-benchmark.ts` (manual-only, mirrors `render-clip.worker.local-comparison.ts`'s harness shape but keeps every AI/vision detector, `@speedora/reframe`, `@speedora/subtitles`, and `../openai` REAL instead of mocked - the entire point being to observe real signals) + `phase-d-report.ts`'s pure `PhaseDReport`/`renderMarkdown()` (8 unit tests) producing a markdown+JSON packet with per-run EditorialDecision/EditPlan/FinalClipQualityAssessment comparison tables, two short-lived presigned review links, and a blank Human Review Rubric explicitly labeled "NOT scored by the agent." A deliberate design simplification versus the original plan: `graphResult` is NOT cached/replayed across the two runs (would need real `jest.spyOn` module-interception risk for a marginal cost saving on a single-clip first pass) - each run makes its own independent real LLM call instead, with the resulting score delta reported as expected LLM-sampling noise. **Run for real once** (2026-08-15, clip `cmsq3ix8a028e2su80g10w5y6`, ~27 minutes): confirmed `editPlan.suggestions`/physical output were byte-identical between flag-off/flag-on (no conflict/budget action fired for this clip - a valid, honestly-reported outcome), and that the entire `editorialScore` delta (53.1→65.8) was attributable to real LLM non-determinism between the two independent Narrative Graph calls, not the flags - validating the design note's own predicted tradeoff with real evidence. Surfaced 2 real, pre-existing findings unrelated to Phases A/B/C1/D's own code, **both fixed** immediately after at the user's request: a real `detect_camera_motion.py` numpy-float32 JSON-serialization bug (`decompose_warp()` now explicitly casts all 4 return values to native `float`, verified with a real ffmpeg-generated test clip through the actual script, no crash), and a sandbox-completeness gap (missing MediaPipe model files + `pytesseract`/`tesseract`, silently null-ing Gesture/Face-Landmark/Object/OCR detection) that led to a real, more consequential discovery: `apps/worker/Dockerfile` never fetched `gesture_recognizer.task`/`efficientdet_lite0.tflite` at all, meaning **production** containers were silently missing Gesture/Object Intelligence too, not just this sandbox - fixed with 2 new Dockerfile `curl` steps (same pattern as its existing 2) plus matching `README.md` prerequisites. **Run 2** (2026-08-16, same clip, after both fixes) offered an initial root-cause guess for `editPlan.suggestions` staying empty (this harness never enabling any `VISUAL_EMPHASIS_*_ENABLED` flag), bumped the harness's own Jest timeout 30→45 min (working detectors take real time), and surfaced + **fixed** a 3rd real bug: `detect_face_landmarks.py`'s Kalman-filter tracker threw a real `cv::gemm` assertion the first time a real model + real trackable face ever reached it (root cause: `cv2.KalmanFilter.statePre`/`.statePost` need a real `(n,1)` column-vector `Mat`, not a flat 1D array - a well-known cv2 Python-binding gotcha; fixed with `.reshape(-1,1)` at both write sites plus a matching `.flatten()` on `predict()`'s own return, verified by reproducing the exact crash on the exact real clip via `git stash`, confirming the fix eliminates it, and a targeted synthetic test driving 3 consecutive `update()` calls to exercise the primary crash path directly). **Run 3** (2026-08-16, after the Kalman-filter fix) corrected Run 2's own root-cause guess as INCOMPLETE: `from-primary-subject-samples.ts`'s `focus_shift` suggestion generation (Phase C1, "data only, always computed") never actually checks `isFocusShiftEnabled()` - that flag only gates whether an already-generated suggestion gets ACTED ON during rendering, never whether `planEdits()` sees it. The real cause of Runs 1-2's empty `editPlan.suggestions` was the Kalman-filter bug starving face-tracking input, not a missing flag. With real face-tracking restored, Run 3 produced this initiative's first genuinely differentiated result: 65 real `focus_shift` + 2 real `reaction_hold` candidates, with `EDIT_BUDGET_ENABLED`/`EFFECT_CONFLICT_RESOLUTION_ENABLED` correctly suppressing 62+1 of them down to real budget caps (117 total decisions, each citing a real `reasonCode`/score/pool size), and the physical render finally differing between flag states (124.76s/`66d40b57...` vs 128.63s/`6bc29be8...`) - real, positive evidence Phase B's arbitration logic works against dense real-world data. Also surfaced and **fixed a 4th real bug, this time in the harness's own code**: `phase-d-report.ts`'s `editPlanSuggestionsIdentical` field compared `off.editPlan.suggestions` vs `on.editPlan.suggestions`, but `Clip.editPlan`'s real persisted shape never includes `suggestions` at all (`packages/edit-plan-director`'s own documented design), so both sides were always `undefined` behind an unchecked `as EditPlanResult` cast - a vacuous comparison silently reporting "identical: yes" every run, caught only because Run 3's real 117-decision table contradicted it. Fixed: renamed to `editPlanArbitrationFired: on.editPlan.decisions.length > 0` (a real, always-correct signal), `editPlan`'s captured type is now honestly `Omit<EditPlanResult, 'suggestions'>`, `phase-d-report.spec.ts` updated to match. New gitignored `apps/worker/reports/` output directory (contains short-lived presigned URLs, never committed). **The blind human evaluation itself then happened for real, once** (Run 3, one reviewer, one clip, collected turn-by-turn over chat and transcribed verbatim into the rubric, never agent-scored): all 5 Editorial questions "yes" (real hook/valuable/narratively complete/understandable standalone/emotional payoff), both Visual questions "yes" (effects purposeful, framing stable), Audio "yes" for clear/well-paced with turn-taking marked N/A (single-speaker clip), and Overall the reviewer preferred the flag-on version and would publish it as-is. Read honestly as one data point, not a generalizable result (§7 of the doc) - it doesn't validate Phase B's arbitration logic at scale, but is a real, non-simulated signal that a human preferred the arbitrated render over the un-arbitrated baseline for this clip. Evaluating Runs 1-2 (whose off/on outputs are byte-identical within each run so had nothing to compare) remains not started; enabling `VISUAL_EMPHASIS_*_ENABLED` flags turned out unnecessary, not merely undone - Run 3 already exercised real arbitration without them. **Multi-clip expansion was attempted and abandoned**: 5 consecutive attempts against a 2nd real clip (the McDonald's video's other real clip, 107.8s) each failed a different way - attempt 1 hit the harness's own 45-min Jest timeout at the final upload/report step (root-caused, not guessed: `ffmpeg.ts`'s pre-existing `TRIM_TIMEOUT_MS=5min` safety net on `trimCutRanges()` correctly firing on this clip's 7 kept segments, can fire in both flag states, fixed by bumping the harness timeout 45→90 min - a real, kept improvement), attempts 2-3 were killed (session-boundary interruptions, no process/resource explanation found), attempt 4 self-skipped on a false-negative `isFfmpegAvailable()` pre-check (ffmpeg verified genuinely working moments later), attempt 5 failed on an `npx`/shell `'jest' is not recognized` resolution error - 5 distinct failure signatures, no common root cause, user-directed hard stop after attempt 5 with explicit instructions not to retry a 6th time or change scope to force a pass. Documented honestly as an unresolved sandbox reliability question for long-lived background jobs, not a pipeline finding - Run 3 remains this initiative's sole real-render, real-human-reviewed evidence and is not weakened by it. See `ai/phase-d-benchmark.md`'s §8. |
...
- **Editorial Director Phase A** ("Editorial Intelligence 2.0") — the first phase of a user-supplied
  30-section "Speedora Editorial Operating System" mission to move clip quality toward Opus Clip
  parity by making the pipeline behave like one editorial decision-maker instead of independently-
  triggering signals. An audit-first pass (not a rebuild) found the diagnosis was accurate — real
  double-counting in `clip-ranking`'s inert `compositeRank`, zero semantic dedup/diversity/
  narrative-boundary logic anywhere, and every v4 signal already free (computed regardless of its
  own flag, ADR D8) — so Phase A is an orchestration layer over already-shipped signals, no new
  detector. **Shipped, flag-gated** (`EDITORIAL_DIRECTOR_ENABLED`, default off): new
  `packages/editorial-director` producing one `EditorialDecision` pre-render (inside
  `candidate-shortlist`, real negative-signal penalties/bounded boundary nudge/diversity-aware
  re-selection) and post-render (inside `render-clip.worker.ts`, always computed per ADR D8, new
  additive `Clip.editorialDecision`, scoring/observability only — does not yet gate rendering).
  Fixes the double-counting structurally via 8 named categories with documented non-overlapping
  source-signal mappings, still HEURISTIC (ADR D4) pending real engagement data. Phase D (real-video
  benchmark/human evaluation) is the roadmap's next phase, explicitly not started. See
  `ai/editorial-director.md`.
- **Edit Plan Director Phase B** ("Edit Orchestration") — the second phase, building on Phase A and
  on a prior, separate initiative's own quantified real-render evidence
  (`visual-emphasis-integration-audit.md`'s Gate B — 531px/s peak crop velocity for Focus Shift×
  Digital Push's worst-case overlap, OCR Highlight's drift measured across movement magnitudes,
  Reaction Hold×Pause Hold confirmed an intentional product dependency). That audit declined to
  build an arbitration rule pending real footage; this phase builds the governing MECHANISM instead,
  informed by those real numbers — a user-confirmed different question, not overriding Gate B's own
  finding. **Shipped, flag-gated** (`EDIT_BUDGET_ENABLED`, `EFFECT_CONFLICT_RESOLUTION_ENABLED`,
  both default off): new `packages/edit-plan-director` — an adaptive per-technique Edit Budget
  (consumes Phase A's own negative-signal output directly, a real cross-phase integration point) and
  an Effect Conflict Resolver implementing 3 pairwise rules grounded in Gate B's evidence
  (time-overlap based, since this layer never sees the actual crop-path pixel positions Gate B's own
  measurements came from — a named, honest limitation) plus budget enforcement. Wired as ONE
  arbitration point feeding all 4 real downstream consumers (confirmed by audit that Pause/Reaction
  Hold bypass `buildReframePlan()` entirely). New additive `Clip.editPlan` column. Hit the documented
  TS2742 Prisma pitfall a second time while wiring this phase (same established fix, verified with a
  real `nest build`). See `ai/edit-plan-director.md`.
- **Render Quality Judge Phase C1** ("Render QA," data only) — the third phase, building on Phases
  A/B. The mission's original Section 17 ask (auto-reject + safe-fallback re-render) isn't safely
  buildable in one pass: a 3-way audit found the 45-minute job timeout has no headroom for a second
  render pass, `renderClip()` is a monolithic non-reusable ffmpeg call, no `Clip` status/verdict
  column exists, and most named Quality Judge inputs (Visual/Audio/Caption quality) have only weak
  proxies or nothing at all — confirmed via audit, not assumed, including a total absence of any
  transcript-to-audio alignment mechanism anywhere in this codebase. User-confirmed scope: ship an
  additive, OBSERVATIONAL `FinalClipQualityAssessment` only, no gating/reject/re-render yet.
  **Shipped, flag-gated** (`RENDER_QUALITY_JUDGE_ENABLED`, default off, gates future API exposure
  only per ADR D8): new `packages/render-quality-judge` — 6 dimensions, each honestly labeled
  `'measured'`/`'proxy'`/`'unavailable'` rather than fabricating a uniform score (editorial/
  narrative/technical quality are real `'measured'` signals composed from Phase A's own
  `EditorialDecision` plus `probeVideoMetadata()`/`RenderVerificationResult`/duration-drift; visual/
  audio quality are explicitly-labeled `'proxy'` scores; caption quality is always `'unavailable'`,
  a confirmed honest gap) composed into one `compositeScore` (proxy weight halved) plus a
  proxy-discounted coverage `confidence`. New additive `Clip.qualityAssessment` column, always
  computed regardless of the flag. Hit the documented TS2742 Prisma pitfall a third time while
  wiring this phase (same established fix, verified with a real `nest build`). Phase C2 (real
  Visual/Audio/Caption quality detectors, auto-reject/re-render) remains next, explicitly not
  started. See `ai/render-quality-judge.md`.
- **Phase D** ("Real-video benchmark, blind human evaluation") — the fourth and final phase, closing
  out Phases A/B/C1. Split via `AskUserQuestion` into an automated benchmark (buildable) and a blind
  human evaluation session (not something an agent can perform) - scope resolved to building the
  harness + a structured human-review packet, leaving the actual evaluation to a human. Audit found
  real video/DB data already live in this dev environment, a previously-undocumented blocker
  (Phases A/B/C1's own migrations had never been deployed to this local Postgres - fixed as a
  prerequisite), that only `EDIT_BUDGET_ENABLED`/`EFFECT_CONFLICT_RESOLUTION_ENABLED` change the
  physical rendered file, and that the real-processor-closure-capture technique only works inside
  Jest. **Shipped and run for real once**: `render-clip.worker.phase-d-benchmark.ts` (manual-only,
  keeps every AI/vision detector and `../openai` REAL, unlike `local-comparison.ts`'s own fully-
  mocked equivalence gate) + `phase-d-report.ts`'s pure report renderer, producing a markdown+JSON
  packet with real EditorialDecision/EditPlan/FinalClipQualityAssessment comparison tables, two
  short-lived presigned review links, and a blank Human Review Rubric explicitly labeled "NOT scored
  by the agent." Run 1 (one clip, ~27 minutes) confirmed `editPlan.suggestions`/physical output
  were byte-identical between flag states for this clip (a valid outcome) and that the entire
  observed `editorialScore` delta was real LLM sampling non-determinism, not the flags - validating
  a deliberate design simplification (graphResult not cached/replayed across runs) with real
  evidence. Surfaced 2 real, pre-existing findings unrelated to this initiative's own code, **both
  fixed** immediately after at the user's request: a `detect_camera_motion.py` numpy-float32
  JSON-serialization bug (verified against a real ffmpeg-generated clip through the actual script),
  and a sandbox-completeness gap (missing MediaPipe model files/`pytesseract`/`tesseract`) that
  surfaced a real production `Dockerfile` gap - `gesture_recognizer.task`/`efficientdet_lite0.tflite`
  were never fetched there at all, silently nulling Gesture/Object Intelligence in production too,
  not just this sandbox - fixed with 2 new Dockerfile `curl` steps + matching `README.md`
  prerequisites. **Run 2** (after both fixes) offered an initial root-cause guess for empty
  `editPlan.suggestions` (this harness never enabling any `VISUAL_EMPHASIS_*_ENABLED` flag), bumped
  the harness's own test timeout 30→45 min, and surfaced + fixed a 3rd real bug:
  `detect_face_landmarks.py`'s Kalman-filter tracker threw a real `cv::gemm` assertion the first
  time a real model + real face ever reached it (root cause: `cv2.KalmanFilter.statePre`/
  `.statePost` need a real `(n,1)` column vector, not a flat 1D array - fixed with `.reshape(-1,1)`
  plus a matching `.flatten()` on `predict()`'s return, verified by reproducing the exact crash on
  the exact real clip via `git stash`, confirming the fix, and a targeted synthetic test of 3
  consecutive `update()` calls). **Run 3** (after the Kalman-filter fix) corrected Run 2's own
  root-cause guess as INCOMPLETE: `focus_shift` suggestion generation
  (`from-primary-subject-samples.ts`, Phase C1 "data only, always computed") never checked
  `isFocusShiftEnabled()` at all - the real cause of Runs 1-2's empty `editPlan.suggestions` was the
  Kalman-filter bug starving it of real face-tracking input, not a missing flag. With real
  face-tracking data flowing again, Run 3 produced this initiative's first genuinely differentiated
  result: 65 real `focus_shift` + 2 real `reaction_hold` candidates, with `EDIT_BUDGET_ENABLED`/
  `EFFECT_CONFLICT_RESOLUTION_ENABLED` correctly suppressing 62+1 of them down to their real budget
  caps (117 total decisions, each citing a real `reasonCode`/score/pool size) and the physical
  render finally differing between flag states (124.76s/`66d40b57...` vs 128.63s/`6bc29be8...`) -
  real, positive evidence that Phase B's arbitration logic works against dense real-world data. Also
  surfaced and **fixed a 4th real bug, this time in the harness's own code**: `phase-d-report.ts`'s
  `editPlanSuggestionsIdentical` field compared `off.editPlan.suggestions` vs `on.editPlan.suggestions`
  via `JSON.stringify`, but `Clip.editPlan`'s real persisted shape never includes `suggestions` at
  all (`packages/edit-plan-director`'s own documented design - it's covered by the pre-existing
  `Clip.editingSuggestions` instead), so both sides were always `undefined` behind an unchecked
  `as EditPlanResult` cast - a vacuous comparison that silently reported "identical: yes" on every
  run, caught only because Run 3's real 117-decision table directly contradicted it. Fixed: the
  field is now `editPlanArbitrationFired: on.editPlan.decisions.length > 0` (a real, always-correct
  signal), `RunResult`/`PhaseDRunSummary`'s `editPlan` field is honestly typed
  `Omit<EditPlanResult, 'suggestions'>`, and `phase-d-report.spec.ts` was updated to match. **The
  blind human evaluation itself then happened for real, once** (Run 3, one reviewer, one clip,
  answers collected turn-by-turn over chat and transcribed verbatim, never agent-scored): all 5
  Editorial questions "yes," both Visual questions "yes," Audio "yes" with turn-taking N/A
  (single-speaker clip), and the reviewer preferred the flag-on version and would publish it as-is.
  One data point, not a generalizable result (see the doc's own §7 for that framing) - it doesn't
  validate Phase B's arbitration logic at scale, but is a real signal that a human preferred the
  arbitrated render over the un-arbitrated baseline for this clip. Enabling
  `VISUAL_EMPHASIS_*_ENABLED` flags turned out to be unnecessary, not merely undone - Run 3 already
  exercised real arbitration without them. **Multi-clip expansion was attempted and abandoned**: 5
  consecutive attempts against a 2nd real clip each failed a different way (a real, understood, now-
  fixed 45→90min timeout headroom issue on attempt 1; 2 kills with no process/resource explanation;
  a false-negative ffmpeg-availability pre-check; an `npx`/shell resolution error) - user-directed
  hard stop after attempt 5, no 6th attempt, no scope changes to force a pass. Documented as an
  unresolved sandbox reliability question, not a pipeline finding; Run 3 remains this initiative's
  sole real-render, real-human-reviewed evidence. See `ai/phase-d-benchmark.md`'s §8.
