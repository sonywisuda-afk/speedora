...
- **Phase D** ("Real-video benchmark, blind human evaluation") — the fourth and final phase, closing
  ...
  by the agent." The real run (one clip, ~27 minutes) confirmed `editPlan.suggestions`/physical
  output were byte-identical between flag states for this clip (a valid outcome) and that the entire
  ...
  prerequisites. **Run for real a 2nd time** after both fixes - corrected the first run's own
  root-cause guess (empty `editPlan.suggestions` is because this harness never enables any
  `VISUAL_EMPHASIS_*_ENABLED` flag, not "reduced detector density"), bumped the harness's own test
  timeout 30→45 min, and surfaced + fixed a 3rd real bug: `detect_face_landmarks.py`'s Kalman-
  filter tracker threw a real `cv::gemm` assertion the first time a real model + real face ever
  reached it (root cause: `cv2.KalmanFilter.statePre`/`.statePost` need a real `(n,1)` column
  vector, not a flat 1D array - fixed with `.reshape(-1,1)` plus a matching `.flatten()` on
  `predict()`'s return, verified by reproducing the exact crash on the exact real clip via
  `git stash`, confirming the fix, and a targeted synthetic test of 3 consecutive `update()`
  calls). Multi-clip expansion, the actual human evaluation session, and enabling Visual Emphasis
  flags for a real `editPlan`-exercising run all remain not started. See `ai/phase-d-benchmark.md`.
