...
- **Phase D** ("Real-video benchmark, blind human evaluation") — the fourth and final phase, closing
  ...
  evidence. Surfaced 2 real, pre-existing findings unrelated to this initiative's own code: a
  `detect_camera_motion.py` numpy-float32 JSON-serialization bug (**fixed** immediately after, at
  the user's request, verified against a real ffmpeg-generated clip through the actual script) and
  a sandbox-completeness gap (missing MediaPipe model files/`pytesseract` in this environment, left
  as backlog). Multi-clip expansion and the actual human evaluation session remain not started. See
  `ai/phase-d-benchmark.md`.
