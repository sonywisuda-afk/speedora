...
- **Phase D** ("Real-video benchmark, blind human evaluation") — the fourth and final phase, closing
  ...
  `detect_camera_motion.py` numpy-float32 bug, missing MediaPipe model files/`pytesseract` in this
  sandbox), logged as backlog, not fixed. Multi-clip expansion and the actual human evaluation
  session remain not started. See `ai/phase-d-benchmark.md`.
