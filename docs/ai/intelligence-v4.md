...
| 13 | Candidate Expansion (10, generation half) — **shipped** (13.1: `CANDIDATE_EXPANSION_ENABLED` prompt fix; 13.2: `@speedora/candidate-shortlist`'s pre-render shortlist stage), see [`ai/clip-ranking-engine.md`](./clip-ranking-engine.md) | Phase 1 | L | Biggest infra change — the new pre-render adapter stage is built; double-paying Stage C to recompute Semantic Events/Narrative Graph WITH grounding for shortlisted survivors is a documented, deliberate scope decision, not solved |
| 14 | Ranking Refinement + Personalization (10 rank half + 11) — **14.1 + 14.2 shipped** (`@speedora/clip-ranking`'s scoring function, wired into `render-clip.worker.ts` as a second additive pass beside the existing "Fase 31" `highlightRank` block), 14.3 (Personalization) not started, see [`ai/clip-ranking-engine.md`](./clip-ranking-engine.md) | Phase 13 + Phases 1-9 | L | Pre-render vs. post-render tension — **ADR D16 resolved** (3-stage funnel, user-confirmed 2026-08-10); 14.2's own risk ("needs a new completion barrier") turned out to be based on an incomplete audit — an equivalent barrier already existed, re-scoping that sub-phase down to S; introduces `WorkspaceContentProfile` schema (D10, Phase 14.3, explicitly deferred out of this delivery's scope), reusing `platform-fit`'s weighted-sum pattern |
