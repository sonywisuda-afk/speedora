...
| 14 | Ranking Refinement + Personalization (10 rank half + 11) — **14.1 shipped** (`@speedora/clip-ranking`'s scoring function), 14.2 (pipeline wiring) and 14.3 (Personalization) not started, see [`ai/clip-ranking-engine.md`](./clip-ranking-engine.md) | Phase 13 + Phases 1-9 | L | Pre-render vs. post-render tension — **ADR D16 resolved** (3-stage funnel, user-confirmed 2026-08-10); introduces `WorkspaceContentProfile` schema (D10, Phase 14.3, explicitly deferred out of this delivery's scope), reusing `platform-fit`'s weighted-sum pattern |
