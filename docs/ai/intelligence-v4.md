...
| 13 | Candidate Expansion (10, generation half) — **audit/ADR/roadmap complete, Phase 13.1 starting**, see [`ai/clip-ranking-engine.md`](./clip-ranking-engine.md) | Phase 1 | L | Biggest infra change — new pre-render adapter stage; must fix `clip-scoring`'s hardcoded "Pick 1-3" prompt text, not just raise `maxCandidates` |
