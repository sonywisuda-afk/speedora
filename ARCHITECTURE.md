...
### VideoImportEngine (adapter/strategy hybrid, config-driven engine registry, and a deliberate one-error-class exception)

Refactors the already-shipped YouTube import pipeline (`apps/worker/src/youtube.ts`, wired end-to-end
since Sprint 1-2) to sit behind a swappable `VideoImportEngine` interface — an internal hardening
pass, not a rebuild: `ImportTabs` → `VideosService.importFromYoutube()` → `import-youtube.worker.ts`
→ engine all stayed the same shape; only what's behind the worker's own download call changed.

- Contract: [`packages/contracts/src/video-import.ts`](./packages/contracts/src/video-import.ts) — `ImportFailureCategory` enum, `VideoImportInput`/`VideoImportMetadata`/`VideoImportResult`.
- Module: [`packages/video-import-engine`](./packages/video-import-engine) — `VideoImportEngine` interface (`fetchMetadata`/`download`/`checkVersion`), today's only implementation `YtDlpEngine` (ports `youtube.ts`'s `spawn`/`execFile` logic behind an injected `ImportDeps`), `resolveEngine(url, config)` (config-driven selection — the reframe-style adapter/strategy shape from the stock-asset `AssetProvider` precedent, `apps/worker/src/assets/`, generalized to a registry so a future `VimeoEngine` etc. needs only a new table row, not a worker rewrite), `withRetry` (generic exponential backoff), and `loadVideoImportEngineConfig(env)` (binary path, timeout, retry/backoff, cookies, proxy, allowed domains, scratch dir, min version — all deployment plumbing kept in `deps`/`config`, never read from `process.env` inside the module itself, same discipline as Reframe's `DetectFacesDeps`).
- **Deliberate deviation from the usual error-class convention**: every other typed error in this codebase (`JobTimeoutError`, `packages/social/src/errors.ts`'s `OAuthNotConfiguredError`) is a small, individually-named `Error` subclass. `VideoImportError` is instead **one** concrete class carrying a `category: ImportFailureCategory` field (`network`/`extractor`/`unavailable`/`private`/`age_restricted`/`rate_limited`/`unsupported`/`timeout`/`cancelled`/`storage`/`internal`) plus `retryable`/`retries`. Chosen because retry-decisioning (`withRetry`) and metrics/health reporting both need to switch on one value across eleven cases — an `instanceof` chain eleven subclasses deep would be worse here, not better. **Lesson for the next module**: the "many small subclasses" convention is a default, not a rule — when a module's failure modes are better modeled as one type with a discriminant field than many nominal types, say so explicitly (as here) rather than force-fitting the usual shape.
- **Cross-process metrics, not in-memory**: `apps/worker` (records) and `apps/api` (reads, via `GET /metrics`) are separate processes, so this couldn't use the in-memory-counter pattern `apps/api`'s own `metrics-registry.ts` uses for HTTP counts. `packages/shared/src/utils/video-import-metrics.ts` defines a structurally-typed `RedisLike` interface (`incr`/`incrby`/`hincrby`/`get`/`set`/`hgetall`) satisfied by both `createRedisConnection()`'s ioredis instance (worker) and a BullMQ `Queue`'s own `.client` (`apps/api`'s `MonitoringController` already pulls one for its Redis health check) — no new Redis connection needed in either process.
- **A real, confirmed gap in `withJobTimeout` this refactor closes**: `jobTimeout.ts`'s own comment already admitted its `Promise.race` "can't force-cancel whatever async work is still in flight" — before this, a job-level timeout never actually killed the in-flight `yt-dlp` child process. Because the timeout is thrown by the *sibling* `timeout` promise in that race, not by the job's own `fn()`, the adapter's own try/catch can never see a `JobTimeoutError` — the fix has to live in a `.catch()` on `withJobTimeout`'s own returned promise (one level up, in the BullMQ processor closure), which calls `controller.abort()` on an `AbortSignal` the engine is already listening on to `SIGTERM`→`SIGKILL` its child process. **Lesson for the next module**: a `Promise.race`-based timeout wrapper can only make its own caller reject promptly — if you need the *loser* of that race to actually stop, the cancellation signal has to be threaded through explicitly, checked from outside the raced function, not caught inside it.
- Adapter: `apps/worker/src/workers/import-youtube.worker.ts` — same orphaned-job/idempotency guards and `Video` status transitions as before (`updateVideoStatus()`), now calling `resolveEngine()` (rejects an unsupported domain before any subprocess spawns) → `engine.fetchMetadata()`/`engine.download()` → `engine.checkVersion()`, recording a `recordVideoImportOutcome()` event on both the success and failure paths.
- Ops tooling deliberately kept outside the module: `apps/worker/src/scripts/yt-dlp-{version,update,rollback}.ts` (checksum-verified manual binary fetch/rollback, run by a human as part of a Docker image rebuild — never invoked by the running worker) follow the existing `apps/worker/src/scripts/*.ts` CLI convention, not a new one.
